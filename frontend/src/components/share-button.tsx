...
export function ShareButton({ questId, questName, onToast, compact = false }: ShareButtonProps) {
  ...
  const xText = `Check out this quest on @autocraft: ${questName} — ${questUrl}`
  const discordText = `**Check out this quest on AUTOCRAFT!**\n📚 **${questName}**\n🔗 ${questUrl}`
  ...
  const handleNativeShare = async () => {
    try {
      await navigator.share({
        title: questName,
        text: `Check out this quest on AUTOCRAFT: ${questName}`,
        url: questUrl,
      })
    } catch {
